...
try {
	...
	//------------------------------- 답글 목록 가져오기 --------------------------------------------------------------------------------
	sql = " SELECT r.created_at, r.content, r.comment_id, r.reply_id, r.created_by, m.nicknm "
		+ " FROM reply r "
		+ " JOIN member m ON m.member_id = r.created_by "
		+ " WHERE r.board_id = ? AND r.delyn = 'N'"
	...
	while (rs.next()) {
		...
		reply.setContent(rs.getString("content"));
		reply.setNicknm(rs.getString("nicknm"));
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.trim() == 'SUCCESS') {
                    ...
                    alert("댓글이 삭제되지 못했습니다. 답글을 먼저 삭제해야 합니다. 관리자에게 문의하세요!");
                }
            },
            error: function () {
                console.log("error");
    ...
    $("#reportModal").on("hidden.bs.modal", function () {
      ...
      $("#iconButton").removeClass("d-none");
			...
			<!------------------------------------------ 작성 내용 출력 ------------------------------------------------>
					<div class="card mb-4">
						<div class="card-body"
							style="background-color: rgba(0, 255, 0, 0.2);">  <!-- 배경색 변경 가능! -->
				...
				<!--------------------------------------------------- 댓글 신고 영역 ---------------------------------------------------->
						<% 
						if (member != null && member.getMemberId() != comment.getCreatedBy()) {	//로그인이 안된 상태이고, 자신의 댓글에는 신고버튼이 안보임!										
						%>
							<div class="text-right">
							...
							<div class="replyArea">
								...
								<div>
									<script>
           							 ...
           							 var commenterNick = '<%=reply.getNicknm()%>';
